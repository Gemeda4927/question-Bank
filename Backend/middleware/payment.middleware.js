...
exports.createCoursePayment = async (req, res, next) => {
  try {
    ...
    if (user.role !== "student")
      return res.status(403).json({ status: "fail", message: "Only students can pay for courses" })
    ...
    const alreadySubscribed = course.subscribedStudents.some(
      (s) => s.studentId.toString() === user._id.toString() && s.coursePaymentStatus === "paid",
    )
    ...
    if (error.response?.data) {
      return res
        ...
        .json({ status: "fail", message: "Payment initialization failed", details: error.response.data })
    }
    next(error)
  }
}
...
exports.handleWebhook = async (req, res) => {
  ...
  try {
    ...
    await course.addStudent(user._id, "paid")

    console.log(`✅ Student enrolled: ${user.email} -> ${course.name}`)
