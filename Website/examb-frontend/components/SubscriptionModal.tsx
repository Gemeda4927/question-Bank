...
export default function SubscriptionModal({
  ...
  const handleCoursePurchase = async () => {
    try {
      ...
      if (result?.data?.checkout_url) {
        ...
        onPurchase(course._id, "full-course");
      } else {
  ...
  const handleExamPurchase = async (
    ...
    try {
      ...
      if (result?.data?.checkout_url) {
        ...
        onPurchase(course._id, "exam", examId);
        setSelectedExamId("");
      } else {
        setError(
          "Failed to initialize exam payment"
        );
      }
    } catch (error: any) {
      console.error(
      ...
      setError(
        error.response?.data?.message ||
          error.message ||
          "Exam payment initialization failed"
      );
    }
  };
  ...
  const handleFreeEnrollment = async () => {
    try {
      setError("");
      // Free courses: send type "course" anyway
      const result =
        await studentService.initializeCoursePayment(
          course._id,
          { type: "course" }
