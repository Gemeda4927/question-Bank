...
interface SubscribedStudent {
  ...
  coursePaymentStatus: "paid" | "unpaid"
  examsPaid: Array<{
    examId: string
...
interface Course {
  ...
  exams?: Exam[] | string[]
  subscribedStudents?: SubscribedStudent[] // Updated type to match backend
...
export default function MarketplacePage() {
  ...
  const extractCoursesData = (response: any): Course[] => {
    ...
    console.warn("[v0] Unexpected API response structure:", response)
    return []
  }
    ...
    try {
      ...
      if (Array.isArray(course.subscribedStudents) && course.subscribedStudents.length > 0) {
        ...
        if (studentSubscription) {
          console.log("[v0] Found subscription for course:", course.name, studentSubscription)

          // Extract paid exam IDs
          const paidExamIds =
            studentSubscription.examsPaid?.filter((exam) => exam.paymentStatus === "paid").map((exam) => exam.examId) ||
            []

          return {
            isEnrolled: true,
            hasPaid: studentSubscription.coursePaymentStatus === "paid",
            enrolledExams: paidExamIds,
          }
        }
      }

      // Fallback to API calls if not found in course data
      const enrollmentRes = await studentService.getEnrollmentStatus(course._id)
      const hasAccess = enrollmentRes.data?.hasAccess || false

      const paymentRes = await studentService.getPaymentStatus(course._id)
      const hasPaid = paymentRes.data?.hasPaid || false
      const enrolledExams = paymentRes.data?.enrolledExams || []
      ...
      console.error("[v0] Error checking enrollment status:", error)
      return {
        isEnrolled: false,
        hasPaid: false,
        enrolledExams: [],
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("[v0] Raw response:", res)
      ...
      for (const course of allCourses) {
        const status = await checkEnrollmentStatus(course) // Pass full course object
        statusMap.set(course._id, status)
        console.log("[v0] Course status:", course.name, status)
      }
  ...
  const handlePurchase = async (
    ...
    try {
      ...
      if ((!course.price || course.price === 0) && purchaseType === "full-course") {
        ...
        toast.success("Successfully enrolled in free course!", { icon: "🎊" })
        ...
        setSubscriptionModal({ course: null, showModal: false })
        return
      }
      ...
      console.error("[v0] Payment/Enrollment error:", error)
                  ...
                  <div>
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                      {exams.map((exam, index) => (
                        <motion.div
                          key={typeof exam === "string" ? exam : exam._id}
                        ...
                        >
                          ...
                          <motion.button
                            ...
                            whileTap={{ scale: 0.95 }}
                            onClick={() =>
                              handlePurchase(course._id, "exam", typeof exam === "string" ? exam : exam._id)
                            }
                            disabled={processingPayment === course._id}
                            className="px-5 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-lg font-semibold text-sm hover:shadow-md transition-all disabled:opacity-50 whitespace-nowrap"
                          >
                            {typeof exam === "string" ? "Buy" : `ETB ${exam.price || "N/A"}`}
                          </motion.button>
                        </motion.div>
                      ))}
                    </div>
          ...
          {loading && courses.length === 0 ? (
            ...
            <motion.div layout className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredCourses.map((course, index) => {
                ...
                const canAccessContent = hasPaid
                      ...
                      <div className="flex justify-between items-center mb-4 relative z-10">
                        ...
                        {canAccessContent && (
                          <motion.span
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="bg-green-400 px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-lg"
                          >
                    ...
                    <div className="p-6 space-y-4">
                            ...
                            {course.exams.map((exam, idx) => {
                              ...
                              return (
                                <div
                                  ...
                                  className="flex items-center justify-between p-3 bg-white border border-blue-200 rounded-xl"
                                >
                                  <div className="flex items-center gap-2">
                                    <Target className="w-4 h-4 text-blue-600" />
                      ...
                      <div className="flex items-center justify-between pt-2">
                        ...
                        {canAccessContent ? (
                          ...
                          >
                            <Eye className="w-5 h-5" /> View Content
                          </motion.button>
                        ) : (
                          <motion.button
