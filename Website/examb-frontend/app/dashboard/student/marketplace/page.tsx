...
export default function MarketplacePage() {
  ...
  const [enrollmentStatus, setEnrollmentStatus] =
    useState<Map<string, EnrollmentStatus>>(
      new Map()
    );
  ...
  ] = useState<SubscriptionModalData>({
    course: null,
    showModal: false,
  });
  ...
  const extractCoursesData = (
    ...
    console.warn(
      "logUnexpected API response structure:",
      response
    );
    return [];
  };
  ...
  const fetchData = async () => {
    try {
      ...
      const res =
        await studentService.getAllCourses();
      ...
      for (const course of allCourses) {
        const status =
          await checkEnrollmentStatus(course);
        statusMap.set(course._id, status);
        console.log(
          "logCourse enrollment status:",
          {
            course: course.name,
  ...
  const handlePurchase = async (
    ...
    try {
      ...
      if (purchaseType === "exam" && examId) {
        response =
          await studentService.initializeExamPayment(
            examId
          );
        toast.loading(
          "Preparing exam payment...",
          { icon: "💳" }
        );
      } else {
        response =
          await studentService.initializeCoursePayment(
            courseId
          );
        toast.loading(
          "Preparing course payment...",
          { icon: "💳" }
        );
      }
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              >
                ...
                Discover amazing courses and level
                up your skills! 🚀
              </motion.p>
            </div>
